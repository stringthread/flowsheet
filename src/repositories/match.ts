...
const isMetaInputObj = (v: unknown): v is MetaInputObj =>
  isObject<MetaInputObj>(v) && ( v.topic===undefined || arrayTypeof(v.topic, 'string') ) && ( v.date===undefined || arrayTypeof(v.date, 'string') )
  && ( v.side===undefined || arrayTypeof(v.side, 'string') ) && ( v.winner===undefined || arrayTypeof(v.winner, 'string') )
  && ( v.opponent===undefined || arrayTypeof(v.opponent, 'string') ) && ( v.note===undefined || arrayTypeof(v.note, 'string') )
  && (v.members===undefined || (Array.isArray(v.members) && v.members.every(
    obj => isObject<{ '_': string, '$': {'part': string} }>(obj) && typeof obj['_'] === 'string' && isObject<{'part': string}>(obj['$']) && typeof obj['$'].part ==='string'
  )));
...
const isMatchInputObj = (v: unknown): v is MatchInputObj => {
  return isObject<MatchInputObj>(v) && (Array.isArray(v['$$']) || v['$$']===undefined) && isObject<MatchInputObj['$']>(v['$']) && typeof v['$'].id==='string';
};
...
export const decodeMatch = (obj: object, idMap: idMap): decodeResult<mMatch> => {
  if(!is_decodeMatchInputObj(obj)) { console.log('not decodeMatchInputObj');return { id: undefined, idMap };}
  ...
  if(Array.isArray(obj.match['$$'])) idMap = obj.match['$$'].reduce((idMap, v) => decodeSide(v, generated.id, idMap).idMap, idMap);
