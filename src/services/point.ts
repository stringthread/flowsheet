...
import {id_is_mPoint, id_is_PointChild, is_mPoint, mPoint,mPointSignature, PointChild, PointParent} from 'models/mPoint';
import {generate_evidence} from './evidence'
import { generate_claim } from './claim';
import { part_add_child } from './part';
import {store} from 'stores';
import { get_parent_id, next_content_id, id_to_store, id_to_type, type_to_store, get_from_id, id_to_slice, compare_id } from './id';
...
export const reorder_child: {
  ...
  if(id_is_mPoint(parent)) store.dispatch(point_slice.actions.reorderChild([parent,target,before]));
    ...
    if(before!==null && !id_is_mPoint(before)) throw TypeError('argument `before` does not match mPoint|null (parent is mPart)');
...
export const append_sibling_point=(parent_id: switch_for_append_id): mPoint|undefined=>{
  return switch_for_append(
    ...
    (id)=>{
      const parent_id=get_parent_id(id);
      if(parent_id===undefined||parent_id===null) return;
      return switch_for_append<mPoint|undefined>(parent_id,
        (parent_id)=>{
          const child=id_is_mPoint(parent_id) ? point_add_child(parent_id, mPointSignature): part_add_child(parent_id);
...
export const set_rebut_to=(from: PointChild['id'])=>(
